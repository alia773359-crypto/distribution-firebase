...
let _refreshInFlight = null; // يمنع عدة محاولات تجديد متزامنة تتنافس على نفس رمز التحديث
async function getValidIdToken() {
  ...
  // لو فيه محاولة تجديد شغّالة فعلًا (من مؤقّت آخر يعمل بالتوازي)، ننتظر نفسها بدل ما نبدأ
  // محاولة ثانية منافسة - Firebase يرفض أحيانًا محاولتين متزامنتين بنفس رمز التحديث، وهذا
  // كان يسبب انقطاع مفاجئ للجلسة رغم إنها لسه سليمة فعليًا.
  if (_refreshInFlight) return _refreshInFlight;
  _refreshInFlight = (async () => {
    try {
      let fresh;
      try { fresh = await authRefresh(s.refreshToken); }
      catch (firstErr) { await new Promise(res => setTimeout(res, 800)); fresh = await authRefresh(s.refreshToken); } // إعادة محاولة واحدة قبل الاستسلام - عطل شبكة عابر ما يستاهل تسجيل خروج كامل
      const latest = getSession() || s; // نقرأ الجلسة الحالية مرة ثانية تحسبًا لتغيّر شي بالأثناء
      const updated = { ...latest, idToken: fresh.idToken, refreshToken: fresh.refreshToken, tokenExpiresAt: Date.now() + Number(fresh.expiresIn) * 1000 };
      setSession(updated);
