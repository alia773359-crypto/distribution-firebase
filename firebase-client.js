...
function branchRowsToAoA(rows) {
  ...
  heads.push('مخزون المصدر قبل', 'المتبقي بالمصدر', 'الوجهة', 'اسم الوجهة', 'مبيعات 3 أشهر الوجهة', 'مبيعات 30 يوم الوجهة', 'مخزون الوجهة', 'في التحويل للوجهة', 'نقل من', 'الكمية المحولة', 'فئة A', 'نوع الأمر', 'الاحتياج', 'هدف التغطية', 'سبب اختيار المصدر', 'تغطية قبل', 'تغطية بعد', 'درجة الخطر', 'سقف التصنيع الخاص');
  const body = rows.map(r => {
    ...
    row.push(r.srcStock, r.srcRemain, r.destId, r.destAr, r.destS90, r.destS30, r.destStock, r.destIncoming || 0, (r.movedFrom !== undefined ? r.movedFrom : 0), r.qty, r.aClass, r.mode, r.need, r.coverTarget, r.reason, r.beforeDays, r.afterDays, r.risk, r.specCap || 0);
    return row;
  });
  return [heads, ...body];
}
...
function shortageWhRowsToAoA(rows) {
  const heads = ['الباركود', 'اسم المنتج', 'فئة المنتج', 'اسم المصدر', 'الوجهة', 'اسم الوجهة عربي', 'مبيعات 3 أشهر الوجهة', 'مبيعات 30 يوم الوجهة', 'مخزون الوجهة', 'في التحويل للوجهة', 'نقل من', 'النقص المحسوب', 'هدف تغطية الأيام', 'مخزون المستودع', 'المتاح من هذا المستودع (فوق حد المستودع)', 'الكمية المقترحة للتحويل', 'سبب اعتبار المنتج ناقص'];
  const body = rows.map(r => [r.barcode, r.name, r.category, r.srcAr, r.destId, r.destAr, r.destS90, r.destS30, r.destStock, r.destIncoming, (r.movedFrom !== undefined ? r.movedFrom : 0), r.need, r.cover, r.srcStock, r.srcAvail, r.qty, r.why]);
  return [heads, ...body];
}
function shortageBrRowsToAoA(rows) {
  const heads = ['الباركود', 'اسم المنتج', 'المصدر', 'اسم المصدر', 'مبيعات 3 أشهر المصدر', 'مبيعات 30 يوم المصدر', 'مخزون المصدر قبل', 'المتبقي بالمصدر بعد الاقتراح', 'الوجهة', 'اسم الوجهة عربي', 'مبيعات 3 أشهر الوجهة', 'مبيعات 30 يوم الوجهة', 'مخزون الوجهة', 'في التحويل للوجهة', 'نقل من', 'النقص المحسوب', 'هدف تغطية الأيام', 'الكمية المقترحة للتوزيع', 'سبب اعتبار المنتج ناقص'];
  const body = rows.map(r => [r.barcode, r.name, r.srcId, r.srcAr, r.srcS90, r.srcS30, r.srcStock, r.srcRemain, r.destId, r.destAr, r.destS90, r.destS30, r.destStock, r.destIncoming, (r.movedFrom !== undefined ? r.movedFrom : 0), r.need, r.cover, r.qty, r.why]);
  return [heads, ...body];
}
