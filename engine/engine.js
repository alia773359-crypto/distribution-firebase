...
function runDistribution(mode, manual, cfg, ctx, onProgress) {
  ...
  rows.forEach((item, idx) => {
    ...
    const dests = selectedDests.map(dest => {
      ...
      // أوضاع التشغيل العادية (Turbo وBIG DATA وغيرها) - وليس فقط بزر "حدود الفروع الخاصة"
      // المستقل. هذا تغيير إضافي بحت: يفعّل فقط للفرع اللي حددت له حدًا أدنى صراحة، ولا يُنقص
      // الاحتياج المحسوب أصلًا أبدًا - فقط يرفعه لو كان أقل من الحد الأدنى المطلوب لذاك الفرع.
      if (!isExceptional && !manual && !isBranchLimitMode && bl && bl.min !== undefined) {
