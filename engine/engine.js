 ...
 */
// ملخّص "نُقل من" لكل منتج - دالة مشتركة (مو محصورة داخل runDistribution) عشان تشتغل من
// التقارير الثلاثة كلها (runDistribution، shortageFromWarehouses، shortageBranchToBranch).
// شيت "نقل من" بالإكسل (ctx.moved) يخزّن رقمًا لكل فرع/مستودع لكل منتج (كم انتقل من هذا
// الموقع)، بدون ذكر الوجهة صراحة داخل نفس الجدول. هذا يبني جملة نصية مختصرة تجمع كل المواقع
// اللي فيها رقم أكبر من صفر بنفس الخلية، مثال: "الهايبر: 6، الاونلاين: 4" - عمود إضافي منفصل
// تمامًا عن "اسم المصدر" (اللي يوضّح مصدر عملية التوزيع الحالية نفسها).
function movedFromSummary(ctx, cfg, b) {
  ...
  rows.forEach((item, idx) => {
      ...
      sources.forEach(so => {
        ...
        while (eligible.length) {
          ...
          eligible.forEach((d, i) => {
            ...
            const record = { runId: rid, barcode: b, name, priority: prio, category: cat, purchaseDate: pur.date, purchaseQty: pur.qty, rec10: dv.rec10, srcId: so.s.id, srcAr: so.s.ar, srcS90: so.sv.s90, srcS30: so.sv.s30, srcStock: so.sv.stock, srcRemain: remainSrc, destId: dest.id, destAr: dest.ar, destS90: dv.s90, destS30: dv.s30, destStock: dv.stock, destIncoming: dv.incoming, movedFrom: movedFromSummary(ctx, cfg, b), qty: unit, aClass: af, mode, need: so.av, coverTarget: cfg.cover, reason, beforeDays, afterDays, risk };
            (so.isWh ? wout : bout).push(record);
    ...
    dests.forEach(d => {
      ...
      for (const so of sources) {
        ...
        const record = { runId: rid, barcode: b, name, priority: prio, category: cat, purchaseDate: pur.date, purchaseQty: pur.qty, rec10: dv.rec10, srcId: so.s.id, srcAr: so.s.ar, srcS90: so.sv.s90, srcS30: so.sv.s30, srcStock: so.sv.stock, srcRemain: remainSrc, destId: dest.id, destAr: dest.ar, destS90: dv.s90, destS30: dv.s30, destStock: dv.stock, destIncoming: dv.incoming, movedFrom: movedFromSummary(ctx, cfg, b), qty: q, aClass: af, mode, need, coverTarget: cfg.cover, reason, beforeDays, afterDays, risk, specCap: sp };
        (so.isWh ? wout : bout).push(record);
        history.push([rid, new Date().toISOString(), b, name, so.s.id, so.s.ar, dest.id, dest.ar, q, mode, reason]);
        register(so.s.id, dest.id, q);
        rem -= q; trans++; qtySum += q; productCovered = true;
  ...
  products.forEach(b => {
    ...
    selectedDests.forEach(dest => {
      ...
      srcAvail.forEach(x => {
        ...
        bySheet[sheetName].push({ barcode: b, name, category: cat, srcAr: x.so.ar, destId: dest.id, destAr: dest.ar, destS90: dv.s90, destS30: dv.s30, destStock: dv.stock, destIncoming: dv.incoming, movedFrom: movedFromSummary(ctx, cfg, b), need: sh.qty, cover: cfg.cover, srcStock: x.sv.stock, srcAvail: x.avail, qty: suggestQty, why: sh.why });
      });
      if (!anyPlaced) {
