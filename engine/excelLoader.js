...
function locFromHeader(h) {
  const text = String(h || '')
    ...
    .split('/')[0].trim();
  // نجرب النص الكامل للعنوان أولًا (فيه أكبر قدر من السياق للتمييز)، ونرجع للجزء الأول بس (قبل
  // الشرطة) كخيار احتياطي فقط لو ما طابق شي. هذا يحل تعارض عمود فرع وعمود مستودع يشتركون بنفس
  // الجزء الأول قبل الشرطة (مثال حقيقي واجهناه: "الرياض نرجس/فرع النرجس" (الفرع) مقابل "الرياض
  // نرجس/مستودع النرجس" (المستودع) - الجزء الأول متطابق حرفيًا بالحالتين، فكان دائمًا يرجّح
  // نفس الموقع بالغلط بغض النظر عن أي عمود فعليًا. النص الكامل يميّز بينهم صح بفضل كلمة
  // "فرع"/"مستودع" بالجزء الثاني، مع خوارزمية "أطول مرادف مطابق يفوز" بدالة locByText.
  return locations.locByText({}, h) || locations.locByText({}, text);
}
